...
def preprocess_pareto(assignments, divs, sizes):
    inst = 0
    for size in sizes:
...
def process_pareto_file(file_name):
    ...
    pareto = exact.get_pareto_front(G, D, n)
    fileio.write_points("pareto/exact/" + file_name, pareto)

def process_approx_file(file_name):
    print("\rCalculating approximate solution: " + file_name)
    (n, G, D) = fileio.load_file("data/" + file_name)
    pareto = algorithm.get_algorithm_points(G, D, n)
    fileio.write_points("pareto/approx/" + file_name, pareto)

# Calculate approximate solutions for given instances
def preprocess_approx(assignments, divs, sizes):
    inst = 0
    for size in sizes:
        for asgn in assignments:
            for div in divs:
                for i in range(10):
                    inst += 1
                    inst_name = div + "_" + str(asgn) + "_"+ str(size) + "_" + str(i)
                    print("\rCalculating approximate solution: " + inst_name + " (" + str(inst) + "/" + str(10 * len(assignments) * len(sizes) * len(divs)) + ")", end='', flush=True)
                    (n, G, D) = fileio.load_file("data/" + inst_name)
                    pareto = algorithm.get_algorithm_points(G, D, n)
                    fileio.write_points("pareto/approx/" + inst_name, pareto)
    print()

def run_test(assignments, divs, sizes):
    preprocess_approx(assignments, divs, sizes)
    preprocess_pareto(assignments, divs, sizes)
...
def calculate_pareto_stats(assignments, divs, sizes):
    pareto_fraction = []
    points_approx = []
    points_exact = []
    inst = 0
        ...
        for div in divs:
            ...
            for size in sizes:
                ...
                for i in range(10):
                    inst += 1
                    inst_name = div + "_" + str(asgn) + "_"+ str(size) + "_" + str(i)
                    print("\rCalculating statistics: " + inst_name + " (" + str(inst) + "/" + str(10 * len(assignments) * len(sizes) * len(divs)) + ")", end='', flush=True)
